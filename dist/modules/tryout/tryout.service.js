...
exports.getRiwayatSesi = exports.getHasilSesi = exports.selesaiTryout = exports.submitSubtes = exports.startSesiTryout = exports.getTryoutById = exports.getTryoutList = exports.deleteTryout = exports.addSoalSubtes = exports.updateStatus = exports.createTryout = exports.hitungBenar = void 0;
...
const hitungBenar = (tipe, kunci, jawaban) => {
    switch (tipe) {
        ...
        case client_1.TipeSoal.SHORT_ANSWER:
            return (String(kunci).trim().toLowerCase() ===
                String(jawaban).trim().toLowerCase());
        default:
            return false;
    }
};
exports.hitungBenar = hitungBenar;
const createTryout = async (input) => {
...
const updateStatus = async (id, newStatus) => {
    const tryout = await prisma_1.prisma.tryout.findUnique({
        where: { id },
        include: {
            subtes: {
                include: {
    ...
    const updated = await prisma_1.prisma.tryout.update({
        where: { id },
...
const addSoalSubtes = async (tryoutId, mapel, soalIds) => {
    ...
    const existCount = await prisma_1.prisma.soal.count({
        where: { id: { in: soalIds } },
    });
    if (existCount !== soalIds.length) {
        return { error: 'some_soal_ids_invalid' };
    }
    let subtes = mapel === 'TPS'
    ...
    if (subtes.mapel !== mapel) {
        subtes = await prisma_1.prisma.subtesTryout.update({
            where: { id: subtes.id },
            data: { mapel },
        });
    }
...
const deleteTryout = async (id) => {
    ...
    await prisma_1.prisma.$transaction([
        prisma_1.prisma.subtesSoal.deleteMany({
            where: { subtesId: { in: subtesIds } },
        }),
        prisma_1.prisma.subtesTryout.deleteMany({
            where: { tryoutId: id },
        }),
        prisma_1.prisma.tryout.delete({
            where: { id },
        }),
    ]);
...
const getTryoutList = async () => {
    const now = new Date();
    // Auto-update status
    await prisma_1.prisma.tryout.updateMany({
        where: {
            status: 'PUBLISHED',
            mulaiAt: { lte: now },
        },
    ...
    const tryouts = await prisma_1.prisma.tryout.findMany({
        where: {
...
const getTryoutById = async (id, role) => {
    const tryout = await prisma_1.prisma.tryout.findUnique({
        where: { id },
        include: {
            subtes: {
...
const startSesiTryout = async (tryoutId, userId) => {
    const tryout = await prisma_1.prisma.tryout.findUnique({
        where: { id: tryoutId },
        include: { subtes: true },
    });
    ...
    const existingSesi = await prisma_1.prisma.sesiTryout.findFirst({
        where: {
            tryoutId,
            userId,
            status: 'IN_PROGRESS',
    ...
    const sesi = await prisma_1.prisma.sesiTryout.create({
        data: {
            tryoutId,
            userId,
            status: 'IN_PROGRESS',
    ...
    const subtesSoals = await prisma_1.prisma.subtesSoal.findMany({
...
const submitSubtes = async (sesiId, userId, jawabans) => {
    const sesi = await prisma_1.prisma.sesiTryout.findUnique({
        where: { id: sesiId },
        include: {
            tryout: {
                include: {
    ...
    for (const ss of subtesSoals) {
        ...
        const benar = userAns
            ? (0, exports.hitungBenar)(ss.soal.tipe, ss.soal.jawaban, ansValue)
            : false;
        if (benar)
            jumlahBenar++;
        jawabanData.push({
    ...
    // Get next subtest
    const nextSubtes = sesi.tryout.subtes.find((s) => s.urutan === currentSubtes.urutan + 1);
    if (nextSubtes) {
        const deadline = new Date(now.getTime() + nextSubtes.durasi * 60 * 1000);
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.jawabanTryout.createMany({ data: jawabanData }),
            prisma_1.prisma.sesiTryout.update({
                where: { id: sesiId },
                data: {
        ...
        // Fetch TKA questions
        const nextSubtesSoals = await prisma_1.prisma.subtesSoal.findMany({
...
const selesaiTryout = async (sesiId, userId, jawabans) => {
    const sesi = await prisma_1.prisma.sesiTryout.findUnique({
        where: { id: sesiId },
        include: {
            tryout: {
...
const getHasilSesi = async (sesiId, userId) => {
    const sesi = await prisma_1.prisma.sesiTryout.findUnique({
        where: { id: sesiId },
        include: {
            tryout: {
...
exports.getHasilSesi = getHasilSesi;
const getRiwayatSesi = async (userId) => {
    const sessions = await prisma_1.prisma.sesiTryout.findMany({
        where: { userId },
...
exports.getRiwayatSesi = getRiwayatSesi;
