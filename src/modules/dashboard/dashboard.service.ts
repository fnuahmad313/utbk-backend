...
export const getSiswaDashboard = async (userId: string) => {
  // === OVERVIEW ===
  ...
  const tryoutSelesai = sesiTryouts.filter(
  ...
  const rataRataSkorLatihan =
    totalLatihan > 0
      ? round1(
          latihanSelesai.reduce((sum, s) => sum + (s.skor ?? 0), 0) / totalLatihan
        )
      : 0

  const tryoutDenganSkor = tryoutSelesai.filter((s) => s.skorTotal != null)
  const rataRataSkorTryout =
    tryoutDenganSkor.length > 0
      ? round1(
          tryoutDenganSkor.reduce((sum, s) => sum + (s.skorTotal ?? 0), 0) /
            tryoutDenganSkor.length
        )
      : 0

  const totalSoalDijawab =
    latihanSessions.reduce((sum, s) => sum + s.jawabans.length, 0) +
    sesiTryouts.reduce((sum, s) => sum + s.jawabans.length, 0)

  // === LATIHAN ANALYTICS ===
  const mapelGroups: Record<
    string,
    { totalSesi: number; skors: number[]; sesiList: { sesiId: string; skor: number; tanggal: string }[] }
  > = {}
  ...
  const perMapel = Object.entries(mapelGroups).map(([mapel, group]) => {
    const rataRataSkor = round1(
      group.skors.reduce((a, b) => a + b, 0) / group.skors.length
    )
  ...
  let kelemahanMapel: string | null = null
  if (perMapel.length > 0) {
    kelemahanMapel = perMapel.reduce((prev, curr) =>
      curr.rataRataSkor < prev.rataRataSkor ? curr : prev
    ).mapel
  }
  ...
  // skorTerbaik
  let skorTerbaik: { sesiId: string | null; judulTryout: string | null; skorTotal: number | null } = {
    sesiId: null,
    judulTryout: null,
    skorTotal: null,
  }
  if (tryoutDenganSkor.length > 0) {
    const best = tryoutDenganSkor.reduce((prev, curr) =>
      (curr.skorTotal ?? 0) > (prev.skorTotal ?? 0) ? curr : prev
    )
    skorTerbaik = {
      sesiId: best.id,
  ...
  // progressDariAwal
  let progressDariAwal: number | null = null
...
export const getAdminDashboard = async () => {
  // === PLATFORM ===
  const [totalUser, totalSoal, totalPTN, totalJurusan, totalTryout, totalTryoutOngoing] =
    await Promise.all([
      ...
      prisma.tryout.count(),
      prisma.tryout.count({ where: { status: 'ONGOING' } }),
    ])

  // === AKTIVITAS BELAJAR ===
  const [totalSesiLatihan, totalSesiTryout] = await Promise.all([
    prisma.latihanSession.count(),
    prisma.sesiTryout.count(),
  ])

  const latihanSelesaiAll = await prisma.latihanSession.findMany({
    where: { selesai: true },
    select: { skor: true },
  })

  const rataRataSkorLatihan =
    latihanSelesaiAll.length > 0
      ? round1(
          latihanSelesaiAll.reduce((sum, s) => sum + (s.skor ?? 0), 0) /
            latihanSelesaiAll.length
        )
      : 0

  const tryoutSelesaiAll = await prisma.sesiTryout.findMany({
    where: {
  ...
  const rataRataSkorTryout =
    tryoutSelesaiAll.length > 0
      ? round1(
          tryoutSelesaiAll.reduce((sum, s) => sum + (s.skorTotal ?? 0), 0) /
  ...
  // === TOP SISWA ===
  const allSesiTryout = await prisma.sesiTryout.findMany({
    where: {
      status: { in: ['SUBMITTED', 'EXPIRED'] },
      skorTotal: { not: null },
    },
  ...
  const siswaMap: Record<
    string,
    { userId: string; nama: string; email: string; skors: number[] }
  > = {}
  ...
  const topSiswa = Object.values(siswaMap)
    .map((s) => ({
      ...
      rataRataSkorTryout: round1(
        s.skors.reduce((a, b) => a + b, 0) / s.skors.length
      ),
    }))
